...
class ProductsDivsincart {
		...
		for(let i = $('.productDisplayed').length; i < myApp.shoppingCart.thingsToBuy.length; i++) { //myApp.displayedProducts

			let eachobjekt = myApp.shoppingCart.thingsToBuy[i] //myApp.displayedProducts[i]
			id = id + 1;
		...
		if (typeof eachobjekt.namn2 == 'object') {
			...
			if ($('.productDisplayed').length == stoppedAt + 15) {
				...
				.on("click",function(e) {
						...
						$('.visafler').remove();
						myApp.productsDivsincart.createDisplayedproducts()
				});
